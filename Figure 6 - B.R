...
otu.tab <- read.table("./DATA/Fiji vaginal and endocervical abundance  data_V4_C.txt", sep="\t", header=TRUE, row.names=1)
dim(otu.tab) # 91 270
otu.tab[1:3,1:3]
...
# 4037   130692  1076828  3035834  3320102 21188675 
...
metadata = read.table("./DATA/Fiji vaginal and endocervical metadata_V5_C.txt", sep="\t", header=TRUE, as.is=TRUE, na.strings=c("N/A", "NA", "."))
dim(metadata) # 91 21
metadata[1:3,]
# Sl.. Sample_ID Age Age.category Clinic                Ethnicity Ct.Status TimePoint Trich..Lab. Trich..Metaphlan. BV..Lab.
# 1    1       20C  21        18-24    oxf iTaukei Pacific Islander  Negative  Baseline    Negative          Negative Positive
# 2    2     20F3C  21        18-24    oxf iTaukei Pacific Islander  Positive  Followup    Negative          Negative Negative
# 3    3       82C  24        18-24    oxf iTaukei Pacific Islander  Positive  Baseline    Negative          Negative Positive
# Candida..Lab. NG..Lab. HPV..HPViewer. HPV.Type MG..VIRGO. subCST     score  CST New.CST  X
# 1      Negative Positive       Negative Negative   Negative   IV-B 0.6866705 IV-B    IV-B NA
# 2      Negative Negative       Positive  Lowrisk   Negative   IV-B 0.6158705 IV-B    IV-B NA
# 3      Negative Negative       Positive  Lowrisk   Negative   IV-B 0.4940242 IV-B   IV-D0 NA
...
table(metadata$HPV.Type, useNA="ifany")
...
metadata$HPV.Type[metadata$HPV.Type=="Highrisk"] = "High risk" 
metadata$HPV.Type[metadata$HPV.Type=="Lowrisk"] = "Low risk" 

### Sankhya: change the variable in the following two lines to any variable after "###"
metadata$var = metadata$Ethnicity ### Ethnicity, New.CST.pooled, MG..VIRGO., HPV.Type, Ct.Status
var.name = "Ethnicity"   
...
### ??? make "strata" part of "data" environment
...
if (var.type %in% c(0, 1)) { 
    ...
    log.chao1.r = lm(log.chao1~1, data=metadata)$residuals+mean(log.chao1)  # Age.category+Ethnicity
    shannon.r = lm(shannon~1, data=metadata)$residuals+mean(shannon)
    #################################################################
    
    pdf(paste("plot_alpha_", var.name, "EC_new.pdf", sep=""), height=4.5, width=9)
    par(mfcol=c(1,2), pty="s", mar=c(5, 4, 4, 2)+0.1)
    
    if (var.type==1) {
        boxplot(log.chao1.r ~ metadata$var, main="log Chao1", ylab="log Chao1", xlab=var.name, xaxt="n") # xaxt="n"
        text(x=1:var.nlevel, par("usr")[3], cex=0.7, labels = var.level, srt = 45, pos = 1, xpd = TRUE, offset=0.8)
        ...
        # pvalue1 <- ldm(log.chao1 ~ var,   # Age.category+Ethnicity
        ...
        legend(x="bottomright", legend=paste("p =", signif(0.007, 3)), bty="n",cex = 1.0)
        ...
        # pvalue2 <- ldm(shannon ~ var,   # Age.category+Ethnicity
        #               data=metadata, seed=67817,
        #               scale.otu.table=FALSE, freq.scale.only=TRUE,
        #               cluster.id=otu.tab.sam, strata=metadata$strata, perm.within.type="free", perm.between.type="free")$p.global.freq
        legend(x="bottomright", legend=paste("p =", signif(0.0226, 3)), bty="n",cex = 1.0)
    } else if (var.type==0) {
...
(p.permanova.BC <- signif(permanovaFL(otu.tab  ~ var, 
...
(p.permanova.J <- signif(permanovaFL(distJ.mean.sq  ~ var, 
...
if (var.type==1) {
    ...
    distBC <- adjust.data.by.covariates(formula= ~ 1, data=metadata, otu.table=otu.tab, dist.method="bray")$adj.dist  # Age.category+Ethnicity
    ...
    pdf(paste("plot_beta_", var.name, ".pdf", sep=""), height=4.5, width=10.5)
    
    par(mfrow=c(1,2), mar=c(5, 4, 4, 5), pty="s", xpd=TRUE)
    plot(distBC.eigen$vectors[,1],distBC.eigen$vectors[,2], main="Bray-Curtis", 
         ...
         col=col[w], pch=pch[w]
         ,xlim=c(min(distBC.eigen$vectors[,1])*1.5, max(distBC.eigen$vectors[,1])*1.5), 
         ylim=c(min(distBC.eigen$vectors[,2])*1.5, max(distBC.eigen$vectors[,2])*1.5)
         )
    ...
    legend(x="topright", inset=c(-0.4,0), bty="n", legend=var.level[level.index], col=col.palette[level.index], pch=pch.palette[level.index])
...
pdf(paste("boxplot_", var.name, "_EC_new.pdf", sep=""), height=6, width=15) 
...
for (i in 1:n.detected.otu.omni) {
    ...
    legend(x="topright", legend=paste("p =", signif(res.ldm$p.otu.omni[o[i]],3)), bty="n",cex=1.0)
}
dev.off()
...
pdf(paste("boxplot_pa_", var.name, "_EC_new.pdf", sep=""), height=6, width=15) 
par(mfrow=c(2,5), pty="s")
for (i in c(1:n.detected.otu.pa)) {
    ...
    text(x=1:var.nlevel, par("usr")[3], cex=0.7, labels = var.level, srt = 45, pos = 1, xpd = TRUE, offset=2.4)
    legend(x="topright", legend=paste("p =", signif(res.ldmA$p.otu.pa[o[i]],3)), bty="n",cex=1.0)
